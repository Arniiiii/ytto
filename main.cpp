...
corral::Task<std::string> typical_http_request(auto& ioc,
                                               ...
                                               Config const& cfg)
{
    ...
    LOG_DEBUG(logger,
              "Doing Jinja stuff to get correct "
    ...
    LOG_DEBUG(logger,
              ...
              magic_enum::enum_name(cfg.method), std::string(cfg.url.path()),
              HTTP_VERSION_TO_USE);
...
corral::Task<std::string> typical_https_request(auto& ioc,
                                                std::string const& request_body,
                                                Config const& cfg)
{
...
corral::Task<std::string> request_to_LLM(auto& ioc, std::string& request_body,
                                         Config const& cfg)
{
    ...
    if (maybe_subtitles.has_value())
    ...
    else
        {
...

            {
                ...
                subtitles_received = co_await get_subtitles(ioc, link_str, cfg);
            }
...
corral::Task<void> async_main(auto& ioc, ABCCache& cache,
        ...
        for (auto& xml_entry : tree.get_child("feed"))
            {
                ...
                LOG_INFO(logger,
                         ...
                         "the link: {}",
                         link_str);
                if (not cfg.proceed_with_shorts and link_str.contains("shorts"))
                    {
                        LOG_INFO(logger,
    ...
    try
        {
            ...
            corral::run(
                ioc, corral::anyOf(async_main(ioc, cache, cache_subtitles, cfg),
